  ...
  for (std::string dependency : packerfile_dependencies) {
    if (packer::is_package_id(dependency)) {
      if (packerfile_index.find(dependency) == packerfile_index.end()) {
        ...
        if (!schedule_package(host, packages, packerfile_index, dependency)) {
          packer::throw_warning(MSG("Dependency '" << dependency << "' of '" << package_id << "' couldn't be schedule"));
          return false;
        }
      } else {
...
bool schedule_package(const packer::Host& host,
  ...
  if (!process_dependencies(host,
    ...
    packer::throw_warning(MSG("couldn't process build dependencies '" << package_id << "'"));
    return false;
  }
  if (!process_dependencies(host,
                            packages,
    ...
    packer::throw_warning(MSG("couldn't process install dependencies '" << package_id << "'"));
    return false;
  }
...
bool packer::schedule_packages(const packer::Host& host,
  ...
  while (!scheduling_queue.empty()) {
    ...
    if (!schedule_package(host, packages, packerfile_index, package_id)) {
